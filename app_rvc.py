import gradio as gr
from dubb.logging_setup import (
    logger,
    set_logging_level,
...
import torch
import os
from dubb.audio_segments import create_translated_audio
from dubb.text_to_speech import (
...
from dubb.translate_segments import (
    translate_text,
    TRANSLATION_PROCESS_OPTIONS,
    DOCS_TRANSLATION_PROCESS_OPTIONS
)
from dubb.preprocessor import (
    audio_video_preprocessor,
    audio_preprocessor,
)
from dubb.postprocessor import (
    OUTPUT_TYPE_OPTIONS,
...
from dubb.language_configuration import (
    LANGUAGES,
...
from dubb.mdx_net import (
    UVR_MODELS,
    MDX_DOWNLOAD_LINK,
    mdxnet_models_dir,
)
from dubb.speech_segmentation import (
    ASR_MODEL_OPTIONS,
    COMPUTE_TYPE_GPU,
    COMPUTE_TYPE_CPU,
    find_whisper_models,
...
from dubb.text_multiformat_processor import (
    BORDER_COLORS,
...
from dubb.languages_gui import language_data, news
import copy
import logging
import json
...
class dubb(dubbTrCache):
    def __init__(self, cpu_mode=False):
        super().__init__()
        if cpu_mode:
            os.environ["DUBBTR_DEVICE"] = "cpu"
        else:
            os.environ["DUBBTR_DEVICE"] = (
                "cuda" if torch.cuda.is_available() else "cpu"
            )

        self.device = os.environ.get("DUBBTR_DEVICE")
    ...
    def multilingual_media_conversion(
        ...
        if not get_video_from_text_json:
            ...
            if not self.task_in_cache("refine_vocals", [vocal_refinement], {}):
                ...
                if vocal_refinement:
                    try:
                        from dubb.mdx_net import process_uvr_task
        ...
        if not self.task_in_cache("acc_and_vc", [
            ...
            if voice_imitation:
                ...
                from dubb.text_to_speech import toneconverter
...
title = "<center><strong><font size='7'> dubb </font></strong></center>"
...
def create_gui(theme, logs_in_gui=False):
    with gr.Blocks(theme=theme) as app:
            ...
            with gr.Row():
                    ...
                    with gr.Column():
                        with gr.Accordion(
                            ...
                            voice_imitation_method_options = (
                                ...
                                if dubbTr.tts_info.xtts_enabled
                ...
                with gr.Column(variant="compact"):
                    ...
                    gr.Examples(
                        ...
                        fn=dubbTr.batch_multilingual_media_conversion,
                        inputs=[
                            video_input,
                            blink_input,
                            directory_input,
                        ...
                        with gr.Row(variant="compact"):
                            ...
                            with gr.Column():
                                tts_test = gr.Dropdown(
                                    sorted(dubbTr.tts_info.list_edge),
        ...
        if dubbTr.tts_info.xtts_enabled:
            ...
            def update_tts_list():
                update_dict = {
                    f"tts_voice{i:02d}": gr.update(choices=dubbTr.tts_info.tts_list())
                    for i in range(MAX_TTS)
                }
        ...
        # Run translate text
        subs_button.click(
            dubbTr.batch_multilingual_media_conversion,
            inputs=[
                video_input,
                blink_input,
                directory_input,
        ...
        # Run docs process
        docs_button.click(
            dubbTr.multilingual_docs_conversion,
            inputs=[
                text_docs,
                input_docs,
                directory_input_docs,
...
if __name__ == "__main__":
    ...
    dubbTr = dubb(cpu_mode=args.cpu_mode)
